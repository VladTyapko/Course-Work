...
<sql:query var="list_purs" dataSource="${DataBase}">
    SELECT * FROM (pur_process pu RIGHT JOIN purchased_product pp ON pu.purchase_id=pp.pur_id) where pur_status='not ready' ORDER BY pur_id;
</sql:query>
<header id="header">
        ...
        <c:forEach var="pur" items="${list_purs.rows}">
            <div class="container">
                ...
                <form method="post" action="${pageContext.request.contextPath}/manage-order">
                    ...
                    <input type="hidden" name="user_email" value="${sessionScope.isAuthorized}">
                    <input type="hidden" name="price" value="${pur.pur_price}$">
                    <input type="hidden" name="purchase_id" value="${pur.purchase_id}">
                    <input type="hidden" name="pur_buyer_email" value="${pur.pur_buyer_email}">
